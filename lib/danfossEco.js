...
class DanfossEco {
    ...
    getTemperatureCurrent = async () => {
        let temperature = await this.getTemperature();

        if (temperature) {
            return temperature.readInt8(1) / 2;
        } else {
            return false;
        }
    }

    setTemperatureTarget = async (value) => {
        let temperature = await this.getTemperature();

        if (temperature) {
            temperature.writeInt8(Math.round(value * 2), 0);

            let result = await this.setTemperature(temperature);
            if (result === false) {
                return false;
            }
    ...
    getTemperatureTarget = async () => {
        let temperature = await this.getTemperature();

        if (temperature) {
            return temperature.readInt8(0) / 2;
        } else {
            return false;
        }
    }
    ...
    setTemperatureInHomeSchedule = async (value) => {
        ...
        if (scheduleData) {
            ...
            let result = await this.setScheduleData_1(scheduleData);
            if (result === false) {
                return false;
            }
    ...
    setAntiFreezeTemperature = async (value) => {
        let settingsData = await this.getSettings();

        if (settingsData) {
            settingsData.writeInt8(Math.round(value * 2), 3);
    ...
    setScheduleOff = async () => {
        let settingsData = await this.getSettings();

        if (settingsData) {
            settingsData.writeInt8(0, 4);
    ...
    setVacationTemperature = async (value) => {
        let settings = await this.getSettings();

        if (settings) {
            settings.writeInt8(Math.round(value * 2), 5);

            let result = await this.setSettings(settings);
            if (result === false) {
                return false;
            }
    ...
    getVacationTemperature = async () => {
        let settings = await this.getSettings();

        if (settings) {
            return settings.readInt8(5) / 2;
        } else {
            return false;
        }
    }

    setVacationDate = async (start, end) => {
        let settings = await this.getSettings();
        ...
        if (settings) {
            ...
            settings.writeInt8(buf1.readInt8(0), 6);
            settings.writeInt8(buf1.readInt8(1), 7);
            settings.writeInt8(buf1.readInt8(2), 8);
            settings.writeInt8(buf1.readInt8(3), 9);

            settings.writeInt8(buf2.readInt8(0), 10);
            settings.writeInt8(buf2.readInt8(1), 11);
            settings.writeInt8(buf2.readInt8(2), 12);
            settings.writeInt8(buf2.readInt8(3), 13);
    ...
    getVacationDateStart = async () => {
        let settings = await this.getSettings();

        if (settings) {
            let dateByte = settings.slice(6, 10);
            let dateInt = dateByte.readInt32BE();
    ...
    getVacationDateEnd = async () => {
        let settings = await this.getSettings();

        if (settings) {
            let dateByte = settings.slice(10, 14);
            let dateInt = dateByte.readInt32BE();
